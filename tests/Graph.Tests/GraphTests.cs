...
public class GraphTests
{
    ...
    public void CanCreateGraphWithoutEdges()
    {
        ...
        graph.GetEdges(node1).Should().BeEmpty();
        graph.GetEdges(node2).Should().BeEmpty();
    }
    ...
    public void CanCreateGraphWithEdges()
    {
        ...
        graph.GetEdges(node1).Should().BeEquivalentTo(new List<Edge>() { edge1 });
        graph.GetEdges(node2).Should().BeEquivalentTo(new List<Edge>() { edge2 });
    }
}
