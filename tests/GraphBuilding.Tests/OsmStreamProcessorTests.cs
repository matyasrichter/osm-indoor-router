...
public class OsmStreamProcessorTests
{
    ...
    public void CanBuildTrivialGraph()
    {
        ...
        {
            new Node { Id = 123456, Latitude = 50.1963895, Longitude = 14.3519902 },
            new Node { Id = 123457, Latitude = 50.1965417, Longitude = 14.3523953 },
            new Way
            {
                Id = 123458,
        ...
        graph.GetEdgesFromNode(nodeA).Should().ContainSingle(x => x.FromId == nodeA.Id && x.ToId == nodeB.Id);
        graph.GetEdgesFromNode(nodeB).Should().ContainSingle(x => x.FromId == nodeB.Id && x.ToId == nodeA.Id);
    }
}
